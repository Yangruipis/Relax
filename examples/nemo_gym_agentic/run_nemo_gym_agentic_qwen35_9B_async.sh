...
#
# Qwen3.5-9B 8xGPU single-node fully-async nemo_gym agentic training script.
#
...
EXP_NAME="qwen35-9B-nemo-gym-agentic-async-${TIMESTAMP}"

if [ -z "${MODEL_DIR:-}" ] || [ -z "${SAVE_DIR:-}" ]; then
    echo "ERROR: MODEL_DIR and SAVE_DIR must be set."
    echo "Example: MODEL_DIR=/path/to/models SAVE_DIR=/path/to/save bash $0"
    exit 1
fi
mkdir -p ${SAVE_DIR}
...
TRAIN_FILES=("'${DATA_PARQUET}'")
TEST_FILES=("'${DATA_PARQUET}'")
...
EVAL_ARGS=(
    --eval-interval 100
    --eval-prompt-data nemo_gym ${TEST_FILES}
